...
const PRESETS = [
  ...
  { label: "Yesterday", get: () => { const y = new Date(); y.setDate(y.getDate() - 1); return { from: startOfDay(y), to: endOfDay(y) }; } },
  ...
  { label: "Last 30 days", get: () => { const d = new Date(); d.setDate(d.getDate() - 30); return { from: startOfDay(d), to: endOfDay(new Date()) }; } },
];
...
const STATUS_BADGE: Record<string, string> = {
  ...
  COMPLETED: "bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-900/20 dark:text-violet-400 dark:border-violet-800",
...
function ChartTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="rounded-xl border border-gray-200/80 bg-white px-3 py-2 shadow-lg dark:border-gray-700/60 dark:bg-gray-900">
      {label && <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-gray-400">{label}</p>}
      {payload.map((p: any) => (
        <p key={p.name} className="text-sm font-semibold tabular-nums" style={{ color: p.color }}>
          {p.name === "Revenue" ? `৳${Number(p?.value)}` : p.value} {p.name !== "Revenue" ? p.name : ""}
        </p>
      ))}
    </div>
  );
...
export default function DashboardOverview() {
  ...
  const { data: overviewRes, isLoading, isError } = useGetDashboardOverviewQuery(queryParams);
  const data: IDashboardOverview | undefined = overviewRes?.data;

  const applyPreset = (preset: (typeof PRESETS)[number]) => {
    const { from, to } = preset.get();
    setCalRange({ from, to }); setDateFrom(from); setDateTo(to); setCalOpen(false);
  };

  const handleCalSelect = (range: DateRange | undefined) => {
    setCalRange(range);
    if (range?.from && range?.to) {
      setDateFrom(startOfDay(range.from)); setDateTo(endOfDay(range.to));
    } else if (range?.from) {
      setDateFrom(startOfDay(range.from)); setDateTo(endOfDay(range.from));
    } else {
  ...
  const clearDate = () => {
    setCalRange(undefined); setDateFrom(undefined); setDateTo(undefined); setCalOpen(false);
  };

  const handleReset = () => {
    setDateFrom(undefined); setDateTo(undefined);
    setCalRange(undefined); setOrderStatus(""); setCalOpen(false);
  };

  const hasFilters = !!dateFrom || !!orderStatus;
  const activeDateLabel = getPresetLabel(dateFrom, dateTo);
  const dateChipLabel = dateFrom ? (activeDateLabel ?? formatDateLabel(dateFrom, dateTo)) : null;
  const activeStatus = ORDER_STATUS_OPTIONS.find(s => s.value === orderStatus);

  const orderStatsChartData = data
    ? [
        { name: "Pending", value: data?.orderStats?.PENDING, fill: "#f59e0b" },
        { name: "Confirmed", value: data?.orderStats?.CONFIRMED, fill: "#10b981" },
        { name: "Completed", value: data?.orderStats?.COMPLETED, fill: "#8b5cf6" },
        { name: "Cancelled", value: data?.orderStats?.CANCELLED, fill: "#ef4444" },
      ]
    : [];
    ...
    <div className="min-h-screen space-y-6 bg-background p-4 md:p-8">
      ...
      <div className="rounded-2xl border border-gray-200/80 bg-white p-4 dark:border-gray-700/60 dark:bg-gray-900 space-y-3">
          ...
          <Select value={orderStatus || "ALL"} onValueChange={(v) => setOrderStatus(v === "ALL" ? "" : v)}>
            ...
            <SelectContent className="rounded-xl">
              {ORDER_STATUS_OPTIONS.map((s) => (
                <SelectItem key={s.value || "all"} value={s.value || "all"} className="cursor-pointer text-sm">
                  <div className="flex items-center gap-2">
                    {s.dot && <span className={cn("h-2 w-2 rounded-full", s.dot)} />}
                    {s.label}
                  </div>
                </SelectItem>
              ))}
          ...
          <Popover open={calOpen} onOpenChange={setCalOpen}>
            <PopoverTrigger asChild>
              <button className={cn(
                ...
                {dateFrom ? <CalendarRange className="h-4 w-4 shrink-0" /> : <CalendarDays className="h-4 w-4 shrink-0" />}
                <span className="truncate max-w-35">
                  {dateFrom ? formatDateLabel(dateFrom, dateTo) : "Filter by date"}
                </span>
                <ChevronDown className={cn("h-3.5 w-3.5 shrink-0 transition-transform duration-200", calOpen && "rotate-180")} />
              </button>
            </PopoverTrigger>
            <PopoverContent align="start" className="w-auto p-0 rounded-2xl border-amber-200/60 dark:border-amber-900/40 shadow-xl overflow-hidden">
              <div className="flex flex-col sm:flex-row">
                ...
                <div className="border-b border-amber-100 dark:border-amber-900/30 sm:border-b-0 sm:border-r sm:w-36 p-3 space-y-0.5">
                  <p className="px-2 pb-1.5 text-[10px] font-bold uppercase tracking-widest text-amber-600/60 dark:text-amber-500/60">Quick select</p>
                  {PRESETS.map((preset) => {
                    const r = preset.get();
                    const isActive = dateFrom && dateTo && isSameDay(r.from, dateFrom) && isSameDay(r.to, dateTo);
                    return (
                  ...
                  {dateFrom && (
                    <>
                      ...
                      <button onClick={clearDate} className="w-full rounded-lg px-2.5 py-1.5 text-left text-xs font-medium text-red-500 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20 transition-colors">
                        Clear date
                      </button>
                    </>
                  )}
                ...
                {/* Calendar */}
                <div className="p-3">
                  <p className="px-1 pb-1.5 text-[10px] font-bold uppercase tracking-widest text-amber-600/60 dark:text-amber-500/60">Custom range</p>
                  <Calendar
                    mode="range" selected={calRange} onSelect={handleCalSelect}
                    numberOfMonths={1} disabled={{ after: new Date() }} initialFocus
                    classNames={{
                      day_selected: "bg-amber-500 text-white hover:bg-amber-500 dark:bg-amber-600",
                      day_range_middle: "bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300",
                      day_range_start: "bg-amber-500 text-white rounded-l-full dark:bg-amber-600",
                      day_range_end: "bg-amber-500 text-white rounded-r-full dark:bg-amber-600",
                      day_today: "border border-amber-400 text-amber-700 font-bold dark:border-amber-600 dark:text-amber-400",
                    }}
                  />
                </div>
              </div>
        ...
        {hasFilters && (
          <div className="flex flex-wrap items-center gap-2">
            {dateChipLabel && (
              <Badge variant="outline" className="flex items-center gap-1.5 rounded-full border-amber-200 bg-amber-50 px-2.5 py-0.5 text-xs font-semibold text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400">
                <CalendarDays className="h-3 w-3" />{dateChipLabel}
                <button onClick={clearDate} className="ml-0.5 hover:text-amber-900 dark:hover:text-amber-200"><X className="h-3 w-3" /></button>
              </Badge>
            )}
            {activeStatus?.value && (
              <Badge variant="outline" className={cn("flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold",
                STATUS_BADGE[activeStatus.value] ?? "")}>
                {activeStatus.dot && <span className={cn("h-1.5 w-1.5 rounded-full", (activeStatus as any).dot)} />}
                {activeStatus.label}
                <button onClick={() => setOrderStatus("")} className="ml-0.5"><X className="h-3 w-3" /></button>
              </Badge>
            )}
          </div>
        )}
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[...Array(isAdmin ? 4 : 2)].map((_, i) => <Skeleton key={i} className="h-28" />)}
        </div>
      ) : isError ? (
        <div className="flex items-center gap-2.5 rounded-2xl border border-red-200 bg-red-50 p-4 dark:border-red-900/40 dark:bg-red-900/10">
          ...
          <p className="text-sm font-medium text-red-700 dark:text-red-400">Failed to load dashboard data. Please try again.</p>
        </div>
      ) : data ? (
        <>
          <div className={cn("grid gap-4", isAdmin ? "grid-cols-2 lg:grid-cols-4" : "grid-cols-2 sm:grid-cols-2")}>
            <StatCard
              label="Total Orders" value={data.totalOrders}
              icon={ShoppingBag} accent="bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400"
            />
            <StatCard
              label="Total Revenue" value={`৳${data.totalRevenue}`}
              icon={TrendingUp} accent="bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400"
              sub="All confirmed payments"
            />
            {isAdmin && data.totalUsers !== undefined && (
              <StatCard
                label="Total Users" value={data.totalUsers}
                icon={Users} accent="bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400"
              />
            )}
            {isAdmin && data.totalProducts !== undefined && (
              <StatCard
                label="Products" value={data.totalProducts}
                icon={Package} accent="bg-violet-50 text-violet-600 dark:bg-violet-900/20 dark:text-violet-400"
              />
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { key: "PENDING", label: "Pending", icon: Clock, cls: "border-amber-200 bg-amber-50/60 dark:border-amber-900/30 dark:bg-amber-900/10", val: "text-amber-700 dark:text-amber-400" },
              { key: "CONFIRMED", label: "Confirmed", icon: CheckCircle2, cls: "border-emerald-200 bg-emerald-50/60 dark:border-emerald-900/30 dark:bg-emerald-900/10", val: "text-emerald-700 dark:text-emerald-400" },
              { key: "COMPLETED", label: "Completed", icon: CheckCircle2, cls: "border-violet-200 bg-violet-50/60 dark:border-violet-900/30 dark:bg-violet-900/10", val: "text-violet-700 dark:text-violet-400" },
              { key: "CANCELLED", label: "Cancelled", icon: XCircle, cls: "border-red-200 bg-red-50/60 dark:border-red-900/30 dark:bg-red-900/10", val: "text-red-700 dark:text-red-400" },
            ].map(({ key, label, icon: Icon, cls, val }) => (
              <div key={key} className={cn("rounded-2xl border p-4 transition-all duration-200 hover:shadow-sm", cls)}>
          ...
          {/* ── Charts Row ── */}
          <div className={cn("grid gap-6", isAdmin ? "grid-cols-1 xl:grid-cols-5" : "grid-cols-1 lg:grid-cols-2")}>

            {/* Pie chart — Order status distribution */}
            <div className={cn("rounded-2xl border border-gray-200/80 bg-white p-5 dark:border-gray-700/60 dark:bg-gray-900", isAdmin ? "xl:col-span-2" : "")}>
              <p className="mb-1 text-sm font-bold text-gray-900 dark:text-gray-50">Order Distribution</p>
              <p className="mb-4 text-xs text-gray-400 dark:text-gray-500">Breakdown by status</p>
              {data.totalOrders === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={orderStatsChartData} dataKey="value" cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3}>
            ...
            {isAdmin && staffBarData.length > 0 ? (
              <div className="rounded-2xl border border-gray-200/80 bg-white p-5 dark:border-gray-700/60 dark:bg-gray-900 xl:col-span-3">
                <p className="mb-1 text-sm font-bold text-gray-900 dark:text-gray-50">Staff Performance</p>
                <p className="mb-4 text-xs text-gray-400 dark:text-gray-500">Top {staffBarData.length} staff by revenue</p>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={staffBarData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              ...
              <div className="rounded-2xl border border-gray-200/80 bg-white p-5 dark:border-gray-700/60 dark:bg-gray-900">
                <p className="mb-1 text-sm font-bold text-gray-900 dark:text-gray-50">Order Status</p>
                <p className="mb-4 text-xs text-gray-400 dark:text-gray-500">Count by status</p>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={orderStatsChartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                    <XAxis dataKey="name" tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} />
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-amber-50/40 dark:border-gray-800 dark:bg-amber-900/5">
                      {["Staff Member", "Email", "Total Orders", "Revenue"].map((h) => (
                        <th key={h} className="px-5 py-3 text-left text-[10px] font-bold uppercase tracking-widest text-amber-700/60 dark:text-amber-500/60">
                          {h}
                        </th>
                      ))}
                    </tr>
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2.5">
                            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[11px] font-bold text-white"
                              style={{ background: `hsl(${[...staff.sellerName].reduce((a, c) => a + c.charCodeAt(0), 0) % 360},52%,50%)` }}>
                              {staff.sellerName?.[0]?.toUpperCase() ?? "?"}
                            </div>
                            <span className="font-semibold text-gray-900 dark:text-gray-50">{staff.sellerName}</span>
                          </div>
                        </td>
                        <td className="px-5 py-3 text-xs text-gray-500 dark:text-gray-400">{staff.email}</td>
                        <td className="px-5 py-3">
                          <Badge variant="outline" className="rounded-full border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400">
                            {staff.totalOrders}
                          </Badge>
                        </td>
          ...
          {data.recentOrders && data.recentOrders.length > 0 && (
            <div className="rounded-2xl border border-gray-200/80 bg-white dark:border-gray-700/60 dark:bg-gray-900 overflow-hidden">
              <div className="border-b border-gray-100 px-5 py-4 dark:border-gray-800 flex items-center justify-between">
                <div>
                  <p className="text-sm font-bold text-gray-900 dark:text-gray-50">Recent Orders</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Last 5 orders matching your filters</p>
                </div>
                  ...
                  return (
                    <div key={order._id ?? idx}
                      ...
                      <p className="shrink-0 font-mono text-xs font-semibold text-gray-700 dark:text-gray-300">
                        {order.customOrderId ? `#${order.customOrderId}` : order._id?.slice(0, 10) + "…"}
                      </p>
                      {/* Customer */}
                      <div className="flex-1 min-w-0">
                        <p className="truncate text-sm font-semibold text-gray-900 dark:text-gray-50">
                          {order.billingDetails?.fullName ?? order.customer?.name ?? "—"}
                        </p>
                        <p className="truncate text-xs text-gray-400 dark:text-gray-500">
                          {order.billingDetails?.email ?? order.customer?.email ?? ""}
                        </p>
                      </div>
                      {/* Status badge */}
                      <Badge variant="outline"
                        className={cn("shrink-0 flex w-fit items-center gap-1.5 rounded-full border text-[10px] font-semibold",
